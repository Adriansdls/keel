...
def is_indexable_dir(cwd: Path) -> bool:
    """Liberal candidate check: True for any non-empty dir that isn't an obvious
    non-project. The real 'is this worth tracking' judgment is made by the LLM
    (and Adrian) when the hint surfaces — not by a rigid filesystem heuristic."""
    home = Path.home()
    # Never hint for home itself or known non-project roots.
    denylist = {
        home,
        home / "Downloads", home / "Desktop", home / "Documents",
        home / "Library", home / "Movies", home / "Music",
        home / "Pictures", home / "Public", home / ".Trash",
        Path("/tmp"), Path("/"),
    }
    if cwd in denylist:
        return False
    # Skip dotfile/hidden config dirs.
    if cwd.name.startswith("."):
        return False
    # Candidate if the dir has any visible content.
    try:
        return any(not c.name.startswith(".") for c in cwd.iterdir())
    except Exception:
        return False
...
def global_briefing(discovery_hint: str | None = None) -> list[str]:
    ...
    if discovery_hint:
        head = [
            "⚠️  REQUIRED FIRST ACTION — UNINDEXED PROJECT",
            f"    {discovery_hint}",
            "    This directory has no .ra-project.yaml. Before responding to anything",
            "    else, judge whether it is a real project worth tracking. If it is,",
            "    your FIRST message this turn MUST ask Adrian: index it now? (yes/no/skip).",
            "    On yes → spawn the ra-pm-init agent with this cwd (full init pipeline:",
            "    identity + 5-layer extraction). On skip → do not ask again this session.",
            "    Do this even if Adrian's prompt is trivial (e.g. \"hi\").",
            "",
        ]
        return head + out
...
def _payload() -> dict:
    """Read the CC hook JSON from stdin exactly once and cache it. stdin can
    only be consumed once, so every consumer (cwd, session_id) shares this."""
    global _PAYLOAD_CACHE, _PAYLOAD_READ
    if _PAYLOAD_READ:
        return _PAYLOAD_CACHE or {}
    _PAYLOAD_READ = True
    try:
        if sys.stdin.isatty():
            _PAYLOAD_CACHE = {}
            ...
            raw = sys.stdin.read()
            _PAYLOAD_CACHE = json.loads(raw) if raw.strip() else {}
    except Exception:
        _PAYLOAD_CACHE = {}
...
def _payload_cwd() -> str | None:
    """cwd from the CC hook payload. Authoritative when present; the habitat
    observer forwards CC's payload here. Falls back to None."""
    try:
...
def main():
    ...
    cwd = Path(_payload_cwd() or os.environ.get("CWD") or os.getcwd()).resolve()
...
def already_shown() -> bool:
    """Dedup once per CC session. Key on the session_id CC sends in the hook
    payload — stable across the whole session. Falls back to ppid only when no
    session_id is present (e.g. standalone invocation)."""
    session = _payload().get("session_id") or f"ppid-{os.getppid()}"
    safe = "".join(c for c in str(session) if c.isalnum() or c in "-_")
    flag = Path(f"/tmp/ra-pm-shown-{safe}")
    if flag.exists():
        return True
    flag.touch()
    return False
