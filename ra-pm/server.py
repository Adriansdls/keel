...
# Add cowork repo root to path so shared/ is importable
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

from shared.store import ra_root as _ra_root

from shared.models import (
    Area, Bet, BetStatus, Claim, ContradictionCheck, Decision, Experiment, ExperimentStatus,
...
def _em_contradicts_llm(new_decision: str, prior_decisions: list[dict]) -> tuple[bool, dict | None, str]:
    """
    LLM-based contradiction check — replaces word-overlap heuristic.
    Returns (contradicts, clashing_prior_or_None, reasoning).
    Fail-open: any LLM error returns (False, None, reason) — never blocks on infrastructure failure.
    """
    if not prior_decisions:
        return False, None, ""
    try:
        from shared.llm import extract, Model
        from shared.models import ContradictionCheck
        prior_text = "\n".join(
            f"[#{p.get('id','?')}] {p.get('decision', '')}"
            for p in prior_decisions[-20:]
        )
        result = extract(
            system_prompt=(
                "You are a decision consistency checker. "
                "Determine whether the NEW DECISION directly contradicts any of the PRIOR DECISIONS. "
                "A contradiction means both cannot simultaneously be honored — not just tension or tradeoffs. "
                "Be conservative: flag only clear logical conflicts, not stylistic differences or minor tension. "
        ...
        if result.contradicts:
            # Find the clashing prior record for the error message
            clash = next(
                (p for p in prior_decisions if result.prior_decision and
                 result.prior_decision[:40].lower() in p.get("decision", "").lower()),
                prior_decisions[-1]  # fallback: most recent
            )
...
def ra_decide(
    ...
    override_contradiction: set true ONLY to deliberately supersede a contradicting prior decision.

    Entropy Manager gate (live): HALTs if this decision contradicts an un-rescinded prior decision —
    convergence is gated, divergence is free. Rescind the prior (a new explicit decision) or pass
    override_contradiction=true if the contradiction is intentional.
    """
    _ensure()
    _priors = _load_records(DECISIONS_DIR, project)
    _contradicts, _clash, _reasoning = _em_contradicts_llm(decision, _priors)
    if _contradicts and not override_contradiction:
        return {"halted": True, "decision": decision,
                "reason": f"contradicts decision #{_clash.get('id')}: {_clash.get('decision', '')[:70]}",
                "reasoning": _reasoning,
                "guidance": "supersede the prior with an explicit rescinding decision, or pass "
                            "override_contradiction=true if this reversal is intentional"}
    d = Decision(
...
@mcp.tool()
def ra_reconcile(apply: bool = False, archive: str = "", snooze_days: int = 0) -> dict:
    """
    Reconcile the project roster against the filesystem. The kernel detects and flags
    (~/.ra/roster-health.json); this tool is the only place mutations happen.

    A missing workspace_path is treated as MOVED, not dead — the kernel scans for the
    project at a new location. Death is never inferred: archiving requires an explicit id.

    - default: report only (states: active / idle / moved / missing / defunct_candidate, plus
      duplicates and untracked markers).
    - apply=True: RELOCATE every 'moved' project to its top relocation candidate
      (update workspace_path; old path kept in known_paths). Duplicates and 'missing'
      projects are surfaced for your decision, never auto-changed.
    - archive="id1,id2": the ONLY path to a terminal/archived state — explicit, human-confirmed.
      Sets status='archived' (reversible, never deleted).
    - snooze_days>0: suppress the session-start roster nudge for N days.
    """
    _ensure()
    health_file = PROJECTS_FILE.parent / "roster-health.json"
    # roster-health.json is JSON (kernel-owned format) — do NOT use the YAML _load/_save helpers.
    try:
        health = json.loads(health_file.read_text()) if health_file.exists() else None
    except Exception:
        health = None
    if not health:
        return {"status": "no roster-health report yet — it regenerates at the next session start (kernel reconcile)."}

    if snooze_days and snooze_days > 0:
        until = (date.today() + timedelta(days=int(snooze_days))).isoformat()
        health["snoozed_until"] = until
        health_file.write_text(json.dumps(health, indent=2))
        return {"status": "snoozed", "snoozed_until": until,
                "note": f"Roster nudge suppressed until {until}. Flags still listed here."}

    entries   = health.get("projects", [])
    by_state: dict = {}
    for e in entries:
        by_state.setdefault(e.get("state", "active"), []).append(e)
    moved     = by_state.get("moved", [])
    missing   = by_state.get("missing", [])
    defunct   = by_state.get("defunct_candidate", [])
    duplicates = [e for e in entries if e.get("duplicate_of")]
    untracked = health.get("untracked", [])

    result: dict = {
        "summary":   health.get("summary", {}),
        "moved":     [{"id": e["id"], "to": (e.get("relocation_candidates") or [{}])[0].get("path")} for e in moved],
        "duplicates": [{"id": e["id"], "duplicate_of": e["duplicate_of"]} for e in duplicates],
        "missing":   [e["id"] for e in missing],
        "defunct_candidate": [e["id"] for e in defunct],
        "untracked": [u.get("path") for u in untracked],
        "applied":   False,
    }

    # Explicit, human-confirmed archival — the only path to a terminal state.
    archive_ids = {x.strip() for x in archive.replace(",", " ").split() if x.strip()} if archive else set()
    if archive_ids:
        projects = _load_projects()
        archived = []
        for p in projects:
            if p.get("id") in archive_ids and p.get("status") != "archived":
                p["status"] = "archived"; p["archived_by"] = "ra_reconcile"; p["archived_at"] = date.today().isoformat()
    ...
    if apply:
        projects = _load_projects()
        relocated = []
        moved_map = {e["id"]: (e.get("relocation_candidates") or []) for e in moved}
        for p in projects:
            cands = moved_map.get(p.get("id"))
            if cands:
                new_path = cands[0].get("path")
                old_path = p.get("workspace_path")
                if new_path and new_path != old_path:
                    kp = p.setdefault("known_paths", [])
                    if old_path and old_path not in kp:
                        kp.append(old_path)
                    p["workspace_path"] = new_path
                    p["relocated_at"]   = date.today().isoformat()
                    relocated.append({"id": p["id"], "from": old_path, "to": new_path})
        if relocated:
            _save(PROJECTS_FILE, projects)
        result["applied"]   = True
        result["relocated"] = relocated
        if duplicates:
            result["duplicates_action"] = (
                "Duplicates are NOT auto-merged (you choose which id survives + where issues/thesis go). "
                "Resolve manually, then archive the loser with ra_reconcile(archive='<id>')."
            )
        if missing:
            result["missing_action"] = (
                "These have no path and no relocation candidate — they need YOUR call: relocate "
                "(register correct path) or archive(id). Never auto-killed."
            )
        if untracked:
            result["untracked_action"] = (
                "Untracked markers were NOT auto-registered (a project needs a thesis/area). "
                "Run the ra-pm-init agent on each path: " + ", ".join(u.get("path", "") for u in untracked)
            )

    # Clear the nudge for what we fixed by forcing a fresh kernel reconcile next read.
    if result["applied"] or archive_ids:
        health["synced_at"] = "1970-01-01"  # force recompute on next roster_health()
        health_file.write_text(json.dumps(health, indent=2))
